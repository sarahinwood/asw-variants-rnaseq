...
# read in data
pca <- fread("output/04_plink/no_ld_filtering/filtered_snps_plink_pca.eigenvec", header = FALSE)
eigenval <- scan("output/04_plink/no_ld_filtering/filtered_snps_plink_pca.eigenval")
...
#fwrite(list(pc1_pos$sample_name), "output/03_plink/pc1_positive_samples.txt")
pc1_neg <- subset(pca_info, PC1<0)
#fwrite(list(pc1_neg$sample_name), "output/03_plink/pc1_negative_samples.txt")
...
pc1 <- subset(long_pca_info, variable=="PC1")
pc2 <- subset(long_pca_info, variable=="PC2")
