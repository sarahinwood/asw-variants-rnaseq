...
vcf_file <- 'output/03_plink/pruned_snps.vcf'

# construct genind
vcf <- read.vcfR(vcf_file)
snp_data <- vcfR2genlight(vcf)

# matrix of allele counts -> impute missing data
na_means <- tab(snp_data, NA.method = "mean")
snps_imputed <- new("genlight",
                    na_means,
                    ploidy = 2)

pops <- gsub("^[[DRI{1}]].+", "\\1", snps_imputed$ind.names)
...
# run the pca on genlight object
